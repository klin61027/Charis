<template>
  <div class="auth-shell">
    <div class="auth-card">
      <div class="accent-bar-org"></div>
      <div class="card-body">

        <div class="back-link" @click="router.push('/login')">
          <IconArrowLeft :size="16" aria-hidden="true" />
          Back to login
        </div>

        <div class="heading">
          <div class="org-badge">
            <IconBuildingCommunity :size="13" aria-hidden="true" />
            Organization
          </div>
          <h2 class="title">Register your org</h2>
          <p class="subtitle">Start posting events and building your community</p>
        </div>

        <form @submit.prevent="submitOrgSignUp" novalidate>

          <div class="field">
            <label for="orgName">Organization name</label>
            <input
              id="orgName"
              v-model="orgName"
              type="text"
              placeholder="e.g. Seattle Green Collective"
              autocomplete="organization"
              class="inp"
            />
          </div>

          <div class="field">
            <label for="contactEmail">Contact email</label>
            <input
              id="contactEmail"
              v-model="contactEmail"
              type="email"
              placeholder="org@email.com"
              autocomplete="email"
              class="inp"
            />
          </div>

          <div class="field">
            <label for="website">
              Website
              <span class="optional">(optional)</span>
            </label>
            <input
              id="website"
              v-model="website"
              type="url"
              placeholder="https://yourorg.org"
              autocomplete="url"
              class="inp"
            />
          </div>

          <div class="field">
            <label for="description">Short description</label>
            <textarea
              id="description"
              v-model="description"
              placeholder="What does your organization do?"
              class="inp inp-textarea"
              rows="3"
            />
          </div>

          <div class="field">
            <label for="password">Password</label>
            <div class="inp-icon">
              <IconLock :size="15" aria-hidden="true" />
              <input
                id="password"
                v-model="password"
                :type="showPassword ? 'text' : 'password'"
                placeholder="Min 8 characters"
                autocomplete="new-password"
              />
              <button
                type="button"
                class="eye-btn"
                @click="showPassword = !showPassword"
                aria-label="Toggle password visibility"
              >
                <IconEye :size="15" aria-hidden="true" />
              </button>
            </div>
          </div>

          <div class="field">
            <label for="confirmPassword">Confirm password</label>
            <div class="inp-icon">
              <IconLock :size="15" aria-hidden="true" />
              <input
                id="confirmPassword"
                v-model="confirmPassword"
                :type="showConfirm ? 'text' : 'password'"
                placeholder="Re-enter password"
                autocomplete="new-password"
              />
              <button
                type="button"
                class="eye-btn"
                @click="showConfirm = !showConfirm"
                aria-label="Toggle confirm password visibility"
              >
                <IconEye :size="15" aria-hidden="true" />
              </button>
            </div>
          </div>

          <Transition name="fade">
            <p v-if="error" class="error-msg">{{ error }}</p>
          </Transition>

          <button
            type="submit"
            class="btn-primary"
            :disabled="isLoading"
          >
            {{ isLoading ? 'Registering…' : 'Register organization' }}
          </button>

        </form>

        <div class="divider">
          <span class="div-line"></span>
          <span class="div-txt">or register with</span>
          <span class="div-line"></span>
        </div>

        <div class="social-row">
          <button
            type="button"
            class="btn-google"
            :disabled="isLoading"
            @click="signUpWithProvider('google')"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" aria-hidden="true">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            Google
          </button>
          <button
            type="button"
            class="btn-apple"
            :disabled="isLoading"
            @click="signUpWithProvider('apple')"
          >
            <IconBrandApple :size="15" aria-hidden="true" />
            Apple
          </button>
        </div>

        <p class="footer-text">
          Already registered?
          <router-link to="/login" class="footer-link">Sign in</router-link>
        </p>

      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { useRouter } from 'vue-router'
import {
  IconArrowLeft,
  IconBuildingCommunity,
  IconLock,
  IconEye,
  IconBrandApple,
} from '@tabler/icons-vue'
import { useOrgSignUp } from '../../viewmodels/useOrgSignUp'

const router = useRouter()

const {
  orgName, contactEmail, website, description,
  password, confirmPassword,
  showPassword, showConfirm,
  isLoading, error,
  submitOrgSignUp, signUpWithProvider,
} = useOrgSignUp()
</script>

<style scoped>
.card-body { padding: 0 28px 36px; }

.accent-bar-org {
  height: 4px;
  background: linear-gradient(90deg, var(--ch-blue) 0%, var(--ch-gold) 100%);
}

.back-link {
  display: flex;
  align-items: center;
  gap: 6px;
  font-size: 13px;
  color: #6b6b72;
  padding: 14px 0 4px;
  cursor: pointer;
}

.heading { padding: 12px 0 20px; }

.org-badge {
  display: inline-flex;
  align-items: center;
  gap: 5px;
  background: var(--ch-blue-soft);
  color: var(--ch-blue);
  font-size: 11px;
  font-weight: 500;
  padding: 3px 10px;
  border-radius: 20px;
  margin-bottom: 14px;
}

.title {
  font-size: 22px;
  font-weight: 500;
  color: #1a1a1a;
  margin-bottom: 4px;
}

.subtitle {
  font-size: 13px;
  color: #a0a0a8;
}

.field { margin-bottom: 13px; }
.field label {
  display: block;
  font-size: 11px;
  color: #6b6b72;
  margin-bottom: 5px;
  font-weight: 500;
}
.optional { color: #c2c2c8; font-weight: 400; }

.inp {
  width: 100%;
  padding: 10px 12px;
  border-radius: 10px;
  border: 0.5px solid #e2e0d8;
  background: #f8f7f5;
  color: #1a1a1a;
  font-size: 13px;
  font-family: var(--font-main);
  outline: none;
  transition: border-color 0.2s;
}
.inp:focus { border-color: var(--ch-blue); }
.inp-textarea { resize: none; line-height: 1.5; }

.inp-icon {
  display: flex;
  align-items: center;
  background: #f8f7f5;
  border: 0.5px solid #e2e0d8;
  border-radius: 10px;
  padding: 0 12px;
  gap: 7px;
}
.inp-icon input {
  flex: 1;
  border: none;
  background: transparent;
  padding: 10px 0;
  font-size: 13px;
  color: #1a1a1a;
  font-family: var(--font-main);
  outline: none;
}
.eye-btn {
  background: none;
  border: none;
  cursor: pointer;
  color: #a0a0a8;
  display: flex;
  padding: 0;
}

.error-msg { font-size: 12px; color: #e24b4a; margin-bottom: 10px; }
.fade-enter-active, .fade-leave-active { transition: opacity 0.2s ease; }
.fade-enter-from, .fade-leave-to { opacity: 0; }

.btn-primary {
  display: block;
  width: 100%;
  padding: 13px;
  border-radius: 12px;
  border: none;
  font-size: 14px;
  font-weight: 500;
  color: #fff;
  background: var(--ch-blue);
  cursor: pointer;
  font-family: var(--font-main);
  margin-top: 4px;
  margin-bottom: 16px;
  transition: opacity 0.2s;
}
.btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }

.divider {
  display: flex;
  align-items: center;
  gap: 10px;
  margin-bottom: 14px;
}
.div-line { flex: 1; height: 0.5px; background: #e2e0d8; }
.div-txt { font-size: 11px; color: #a0a0a8; }

.social-row { display: flex; gap: 8px; margin-bottom: 16px; }
.btn-google {
  flex: 1;
  padding: 10px;
  border-radius: 10px;
  border: 0.5px solid #dadce0;
  background: #fff;
  font-size: 11px;
  font-weight: 500;
  color: #3c4043;
  cursor: pointer;
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 7px;
  font-family: var(--font-main);
}
.btn-apple {
  flex: 1;
  padding: 10px;
  border-radius: 10px;
  border: 0.5px solid #1a1a1a;
  background: #1a1a1a;
  color: #fff;
  font-size: 11px;
  font-weight: 500;
  cursor: pointer;
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 7px;
  font-family: var(--font-main);
}
.btn-google:disabled, .btn-apple:disabled { opacity: 0.6; cursor: not-allowed; }

.footer-text {
  text-align: center;
  font-size: 12px;
  color: #a0a0a8;
}
.footer-link {
  font-weight: 500;
  text-decoration: none;
  color: var(--ch-blue);
}
</style>
